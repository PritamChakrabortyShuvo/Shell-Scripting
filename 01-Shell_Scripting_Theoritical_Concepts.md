...
## Key Points About `bash +x`
1. **Debug Mode** :
    - Enables the debugging feature in Bash.
    - Shows the commands being executed along with any substitutions (e.g., variables, expansions).
2. **Syntax** :
```bash
...
## Use Case
Use bash **`+x`** to debug when:
- A script is not working as expected.
- You want to see how variables, loops, or conditions are evaluated step-by-step.
# First Shell-Script
...
```bash
    case <variable or value> in
        ...
        pattern2)
            ...
            ;;
        pattern3|pattern4)
            # Commands to execute if pattern3 or pattern4 matches
            ;;
        *)
...
```output
    ...
    Exiting...
```
## Example 02 : File Selection Menu
```bash
    #!/bin/bash

    echo "Select a file to display its content:"
    select file in *
    do
        if [ -f "$file" ]; then
